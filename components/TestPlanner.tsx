...
// --- Reusable UI Components (shared style with Planner) ---
...
const Button: React.FC<React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: 'primary' | 'secondary' | 'danger' | 'ghost' }> = ({ children, className, variant = 'primary', ...props }) => {
    const baseClasses = "px-2 py-1 rounded-md font-semibold transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed text-xs";
    const variantClasses = {
        primary: "bg-brand-cyan-500 text-brand-blue-900 hover:bg-brand-cyan-400 shadow-glow-cyan-light hover:shadow-glow-cyan px-4 py-2 text-sm",
...
const Modal: React.FC<{ isOpen: boolean; onClose: () => void; title: string; children: React.ReactNode; maxWidth?: string }> = ({ isOpen, onClose, title, children, maxWidth = 'max-w-2xl' }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] animate-fadeIn" onClick={onClose}>
            <div className={cn("bg-brand-blue-900/80 border border-brand-cyan-700 rounded-lg shadow-glow-cyan p-6 w-full", maxWidth)} onClick={e => e.stopPropagation()}>
        ...
        <Modal isOpen={isOpen} onClose={onClose} title="Plan a New Test">
            <form onSubmit={handleSubmit}>
                ...
                <div className="flex justify-end gap-2 mt-6">
                    <Button type="button" variant="secondary" className="px-4 py-2 text-sm" onClick={onClose}>Cancel</Button>
        ...
        <Modal isOpen={!!test} onClose={onClose} title={`Analyze: ${test.name}`} maxWidth="max-w-lg">
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    ...
                    <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={4} className="w-full bg-white/10 dark:bg-black/30 border border-white/20 rounded-md px-3 py-2 text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-brand-cyan-500" placeholder="e.g., Made silly mistakes in Physics, need to revise Chemistry formulas..."></textarea>
                </div>
                <div className="flex justify-end gap-2 mt-6">
                    <Button type="button" variant="secondary" className="px-4 py-2 text-sm" onClick={onClose}>Cancel</Button>
            ...
            <div className="space-y-4">
                ...
                <div>
                 ...
                 <div className="flex justify-end mt-6">
                    <Button onClick={onClose} variant="secondary" className="px-4 py-2 text-sm">Close</Button>
                </div>
            </div>
        </Modal>
    );
                            ...
                            <Card key={test.id} className="p-4 flex flex-col gap-4">
                                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center w-full">
                                    ...
                                    <div className="flex items-center gap-2 self-end sm:self-center">
                                        <Button onClick={() => setExpandedTest(expandedTest === test.id ? null : test.id)} variant="secondary">
                                            {expandedTest === test.id ? 'Hide Prep' : 'Start Prep'}
                                        </Button>
                                        <Button onClick={() => setTestToAnalyze(test)} variant="secondary">Mark as Complete</Button>
                                    ...
                                    <div className="w-full bg-black/20 p-4 rounded-lg max-h-[70vh] overflow-y-auto space-y-6">
                                        ...
                                        <div>
                                            ...
                                            <table className="w-full text-xs">
                                                <thead className="sticky top-0 bg-black/30 z-20">
                                                                            ...
                                                                            <tr key={topic.microtopic} className="border-b border-white/10 hover:bg-white/5">
                                                                                ...
                                                                                <td className="py-1.5 text-center">
                                                                                    {topic.revisionDifficulty ? (
                                                                                        ...
                                                                                        <Button variant="ghost" onClick={() => { setCurrentTestId(test.id); setTopicToUpdate(topic); setRevisionModalOpen(true); }}><BrainIcon className="w-4 h-4 mr-1"/>Revise</Button>
                                                                                    )}
                                                                                </td>
                                                                                <td className="py-1.5 text-center">
                                                                                    {avgAccuracy !== null && (
                                                                                    ...
                                                                                    <Button variant="ghost" onClick={() => { setCurrentTestId(test.id); setTopicToUpdate(topic); setPracticeModalOpen(true); }}><ClipboardCheckIcon className="w-4 h-4 mr-1"/>Practice</Button>
                                                                                </td>
                                                                            </tr>
                                                                        );
                                                                    })}
                         ...
                         {completedTests.map(test => (
                            <Card key={test.id} className="p-4">
                                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                                    ...
                                    <div className="flex items-center gap-2 self-end sm:self-center">
                                        <Button onClick={() => setViewingTest(test)} variant="secondary">View Analysis</Button>
                                        <button onClick={() => deleteTest(test.id)} className="p-1 text-gray-400 hover:text-red-400 transition-colors"><Trash2Icon className="w-4 h-4" /></button>
                                    </div>
                                </div>
                            </Card>
